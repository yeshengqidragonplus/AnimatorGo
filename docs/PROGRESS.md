...
这几个小件缩放差 50% 却留在 SpriteSkin 路径。~~ **查下来不是缺口**(2026-09-23):这句当时只看了缩放比,
没看另一条判据。这几个小件的绑定残差 5~19 像素,一直被「绑定姿势非刚性」分流到 SkinnedMeshRenderer。
MC2 全部 31 个「跨度 < 64、缩放差 > 2%」的网格:24 个残差超标已分流,剩下 7 个缩放差 × 跨度都 < 1 像素、
残差 < 0.5 像素 —— 是整数裁剪框的量化噪声,留在 SpriteSkin 是对的(挪去 SkinnedMeshRenderer 反而会丢 slot 颜色动画)。
加了回归用例钉住。

全盘复查(8098 个骨架、22221 个留在 SpriteSkin 的混合加权网格)用的是**与姿势无关的误差上界**
`max_v Σ wᵢ |fitᵢ(lᵢ/k) − v|` —— Unity 与 Spine 的顶点差 = Σ wᵢ Bᵢ(t)·eᵢ,骨骼刚体时模长不超过它。
setup 残差是各影响**加权后**的,误差可以互相抵消,动起来就不抵消了,所以只看 setup 不够。
跨度 < 64 的最大上界 **1.4 像素**(包括缩放比差 9% 的 `hair/9_behind` 这类 —— 「缩放差 × 跨度」是粗估,
逐骨骼拟合吸收了大半)。

**上界太松,判据不用改**:上界超阈值的 15 个网格(多为跨度 ≥ 64),拿姿势求值器把每条动画按 1/60 秒逐帧算
Unity 与 Spine 的顶点差(两边同一套骨骼姿势,只量绑定这一项):

| 网格 | 上界 | 动画实测最大 | setup 残差 |
|---|---|---|---|
| `clown_npc` / `biyan`(78px) | 9.45 | **1.00** | 1.00 |
| `fan` / `fan2_3`(99px) | 8.51 | **0.76** | 0.76 |
| `daoju_tanhuang` / `1`(236px) | 7.69 | **1.49** | 1.93 |

14 个动画里都不超阈值,实测几乎就等于 setup 残差 —— 各影响的误差方向高度相关,动起来并不「不抵消」。
唯一超的是 `7051_03_19` 那块 621px 的地毯:`first_confirm` 第 0 帧 14.1px(阈值 12.4),那一刻骨骼放大了,
误差跟着放大,占显示尺寸的比例仍约 2%。**所以残差判据保持 setup 残差不变。**
...
1. **Unity → Spine**(反方向)
2. **Godot / Cocos 导出**
3. `.skel` 里往返验不出的区域:path 约束里同类型字段的语义顺序(字节布局已由 99 个真实文件确认,
   见 [SPINE-BINARY.md](SPINE-BINARY.md) 第 9 节)
4. **VAT 出口** —— 极限性能时才需要。来源有两个:Spine 直出,以及 Unity 正常动画烘焙。
   求值器按「(数据, 时间) → 顶点数组」设计,让两边共用

~~跨度 < 64px 的加权网格不参与缩放分流~~ 查下来不是缺口,见上文。
...
| 问题 | 修法 | 验证 |
|---|---|---|
| 同一属性的重复时间轴(3.8 常见)`skel → json` 静默留最后一条 | 抽成 `spine-format/duplicateTimelines.ts`:只留最后一条(Spine 实际播出来的那条),**内容不同的**报 approximated。全盘约 4800 处里 4500 处前后两条内容完全相同,不报 | 全盘 8211 个 `.skel`:内容不同的 297 处、52 个文件,每处一条报告 |
| ⭐ **同一问题在 Unity 导出里是看得见的 bug**:两条 deform 各自变成 Blend Shape,Blend Shape 是**叠加**的 → 形变翻倍 | Unity 导出同样只取最后一条 | 实测 MC2 的 Juicer:`work` 里 5 个网格各有 `work_0` + `work_0_2` 同时满权重,修后没了(blendShape 曲线 35 → 30);合成用例要求插入重复后产物**逐字节不变** |
| JSON 事件帧省略的 `int` / `float` 按 0 读写 | 缺省取**事件定义**的值 —— Spine JSON 规范原文 "Assume the setup pose value if omitted"(全盘真实 JSON 去重后只有 35 个,没有一帧引用非 0 定义,只能按规范) | 全盘 `.skel → .json → 读回`,677 个事件帧(51 个引用非 0 定义)逐项相等;真实 JSON 写回仍 117/117、12/12 |
