...
📌 **顺带发现、未处理**:跨度 ≥ 64 的有 10 个网格 setup 残差在阈值内、上界却 2~9 像素
(`clown_npc` 的 `biyan` 78px:setup 1.00、上界 9.45;`fan` 的 `fan2_3`:0.76 / 8.51)。上界是最坏情况,
动画里实际偏多少要拿姿势求值器逐帧算过才知道 —— 若确实可见,把残差判据换成上界即可(只影响这 10 来个)。
